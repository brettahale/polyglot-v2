...
const SettingsSchema = mongoose.Schema({
  ...
  'mqttBindPort': {
      type: String,
      default: process.env.MQTT_BIND_PORT || null
  },
  'mqttBindAddress': {
      type: String,
      default: process.env.MQTT_BIND_ADDRESS || null
  },
  'mqttHost': {
    type: String,
...
SettingsSchema.statics = {
  ...
  async parseEnvUpdate() {
    try {
      ...
      if (settings.hasOwnProperty('USE_HTTPS')) { config.settings.useHttps = settings.USE_HTTPS !== 'false' } else { config.settings.useHttps = true }
